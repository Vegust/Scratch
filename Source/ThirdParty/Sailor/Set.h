...
namespace Sailor
{
	template<typename TElementType, typename TAllocator = Memory::DefaultGlobalAllocator, const size_t ReservedElements = 6>
	class TSet
	{
	public:
	...
	protected:
		...
		__forceinline bool ShouldRehash() const
		{
			...
			return (size_t)m_num > (m_buckets.Num() * ReservedElements) / 3;
		}
